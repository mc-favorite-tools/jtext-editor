...
import PresetColor from '../../core/tellraw/color';
...
const basicColors = PresetColor.map(color => color.fc)
const WIDTH = 214;
const HEIGHT = 150;

export default function ColorPicker(props: ColorPickerProps): JSX.Element {
    const [selfColor, setSelfColor] = useState(transformColor('hex', props.color));
    ...
    return (
        <Dropdown className='dropdown'onVisibleChange={handleVisibleChange} visible={visible} overlay={<div
            ...
            ref={innerDivRef}>
            <Input prefix='Hex' onChange={onSetHex} value={inputColor} />
            <div className="color-picker-basic-color">
                {basicColors.map((basicColor, index) => (
                    <button
                        title={PresetColor[index].id}
